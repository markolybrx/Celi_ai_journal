...
function openRanksModal() { 
    ...
    // FIX: Explicitly set display: flex to ensure it opens after being closed
    ...
    // Grab the current Psyche from the dashboard to inject into the tree
    ...
    globalRankTree.ranks.forEach((rank, index) => { 
        ...
        // V9.3: Added Synthesis Injection
        let synthesisHtml = '';
        if (isCurrent) {
            synthesisHtml = `<div class="tree-synth">${currentPsyche}</div>`;
        }
